...
fn run_command(uart: &mut Uart, line: &str, history: &[String], ramdisk: &Option<TarFileSystem<'static>>) {
    ...
    match cmd {
        ...
        "cat" => {
            ...
            if let Some(fs) = ramdisk {
                if let Some(file) = fs.find(target_file) {
                    if file.is_dir() {
                        let _ = writeln!(uart, "Error: '{}' is a directory.", target_file);
                    } else if let Some(text) = file.as_text() {
                        let _ = write!(uart, "{}", text);
                        // Add a trailing newline if the file text doesn't have one naturally
                        if !text.ends_with('\n') {
                            let _ = writeln!(uart);
                        }
                    } else {
                        let _ = writeln!(uart, "Error: File '{}' is binary or invalid UTF-8.", target_file);
                    }
                } else {
                    let _ = writeln!(uart, "Error: File '{}' not found.", target_file);
                }
            } else {
                let _ = writeln!(uart, "Error: No ramdisk mounted.");
            }

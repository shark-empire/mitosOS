...
const MAGIC_CANARY: u64 = 0xDEAD_BEEF_CAFE_BABE;
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let ptr = if let Some(index) = target_bucket_index(&layout) {
            if !allocator.buckets[index].is_null() {
                ...
                allocator.buckets[index] = (*node).next;
                node as *mut u8
            } else {
        ...
        if !ptr.is_null() {
            ptr::write_bytes(ptr, 0, size); // Zeroing for security
        }
        ptr
    }
...
// =========================================================================
// 4. PHYSICAL MEMORY MANAGER (Simplified for brevity)
// =========================================================================

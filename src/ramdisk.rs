...
impl<'a> TarFileSystem<'a> {
    ...
    pub unsafe fn new(base_addr: usize, size: usize) -> Option<Self> {
        ...
        // NOTE: no trailing semicolon here — this must evaluate to the slice,
        // not to `()`. That semicolon was the root cause of every downstream
        // "expected &[u8], found ()" error.
        let data = unsafe { core::slice::from_raw_parts(base_addr as *const u8, size) };
